...
public class CourseManagerFrame extends JFrame {
    ...
    private JTextField deadlineField = new JTextField(18);
    ...
    private JComboBox<String> timeBox = new JComboBox<>(
            new String[] {
                    "09:05-10:35", "10:45-12:15", "12:25-13:55",
    ...
    public CourseManagerFrame() {
        ...
        // Table
        String[] cols = {
                "ID", "Code", "Title", "Day", "Time", "Room",
                "Max", "Available", "Drop Deadline"
        };
        ...
        c.gridx = 1;
        form.add(maxSeatsField, c);
        y++;

        c.gridx = 0;
        c.gridy = y;
        form.add(new JLabel("Available Seats"), c);
        c.gridx = 1;
        form.add(availableSeatsField, c);
        y++;

        c.gridx = 0;
        c.gridy = y;
        form.add(new JLabel("Drop Deadline (YYYY-MM-DD)"), c);
        c.gridx = 1;
        form.add(deadlineField, c);
        y++;
    ...
    private void load() {
        ...
        deadlineField.setText(model.getValueAt(r, 8).toString());
    }

    private void addCourse() {
        try (Connection con = DBConnection.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO courses(course_code,course_title,day,time_slot,room,max_seats,available_seats,drop_deadline) "
                            +
                            "VALUES (?,?,?,?,?,?,?,?)");
            ps.setString(1, codeField.getText().trim());
            ...
            ps.setDate(8, Date.valueOf(deadlineField.getText().trim()));
            ps.executeUpdate();
            search();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
    ...
    private void updateCourse() {
        ...
        try (Connection con = DBConnection.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "UPDATE courses SET course_title=?,day=?,time_slot=?,room=?,max_seats=?,available_seats=?,drop_deadline=? "
                            +
                            "WHERE course_id=?");
            ...
            ps.setDate(7, Date.valueOf(deadlineField.getText().trim()));
            ps.setInt(8, selectedCourseId);
            ps.executeUpdate();
            search();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, e.getMessage());
